...
8. Agendamento = **visita com itens** (`Appointment` + `AppointmentItem` por serviço/profissional); comissão apurada por item ao concluir; perfis OWNER/MANAGER/RECEPTION/STAFF em `requireAuth`. Regras puras em `src/lib/**` com testes em `tests/` (`npm test`).
