...
import { CATEGORY_EXAMPLES, CATEGORY_ICONS, CATEGORY_LABELS, CATEGORY_ORDER } from "@/lib/services/categories";
...
// Durações típicas do segmento (seção 6.2): manicure 45, gel 1h15, manutenção 2h, alongamento 2h30.
...
export function ServiceForm({ initial, defaultAddOn }: { initial?: ServiceFormValues; defaultAddOn?: boolean }) {
  ...
  const [category, setCategory] = useState<ServiceCategory>(initial?.category ?? (isAddOn ? "ADICIONAL" : "MANICURE"));
    ...
    <form action={formAction} className="card max-w-xl space-y-4 p-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <TypeOption
            selected={!isAddOn}
            onClick={() => { setIsAddOn(false); if (category === "ADICIONAL") setCategory("MANICURE"); }}
            title="Procedimento"
            hint="A cliente agenda sozinho (ex.: manicure, alongamento)"
          />
          <TypeOption
            selected={isAddOn}
            onClick={() => { setIsAddOn(true); setCategory("ADICIONAL"); }}
            title="Adicional"
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_200px]">
        <div>
          ...
          <input id="name" name="name" required className="input" defaultValue={initial?.name ?? ""} list="service-examples" placeholder={isAddOn ? "Nail art" : "Alongamento em fibra de vidro"} />
          {examples.length > 0 && (
        ...
        <div>
          ...
          <select id="category" name="category" className="input" value={category} onChange={(e) => setCategory(e.target.value as ServiceCategory)}>
