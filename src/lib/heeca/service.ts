...
export async function provision(e: Entitlement) {
  const existing = await db.tenant.findUnique({ where: { heecaSubscriptionId: e.subscriptionId }, select: { id: true, slug: true } });
  if (existing) {
  ...
  if (taken) throw new Error(`O e-mail ${email} já é usuário do estabelecimento "${taken.tenant.slug}" no Heeca Beauty.`);
...
export async function resolveSsoUser(c: SsoClaims) {
  ...
  if (user && user.tenantId !== tenant.id) throw new Error("Este e-mail já é usuário de outro estabelecimento no Heeca Beauty.");
  if (!user) {
